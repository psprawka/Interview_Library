...
**  as possible, and interprets them as a numerical value. The string
**  can contain additional characters after those that form the integral
**  number, which are ignored and have no effect on the behavior of this
**  function. If the first sequence of non-whitespace characters in str
...
**  the minus sign. Then take as many numerical digits as possible,
**  which gets 42.
**  
**  Example 3:
...
**  not a numerical  digit or a +/- sign. Therefore no valid conversion
**  could be performed.
**
**  Example 5:
**  Input: "-91283472332"
...
**  a 32-bit signed integer. Thefore INT_MIN (−231) is returned.
*/
