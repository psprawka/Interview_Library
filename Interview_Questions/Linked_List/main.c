...
/*   Updated: 2018/10/07 23:21:23 by psprawka         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	t_node	*head1, *head2, *tmp, *inter = NULL;
	t_data	*data;
	int		len, len2;
	...
	{
		data = init_data(rand() % 10);
		tmp = ft_init_node(data, sizeof(t_data));
		ft_add_back_list(&head1, tmp);
		
		if (len > len2/2)										/*	intersection done here	*/
		{
			inter = tmp;
			tmp = ft_init_node(data, sizeof(t_data));
			ft_add_back_list(&head2, tmp);
		}
	}
	...
	printf("Intersection should be at: %d\n", inter ? ((t_data *)inter->data)->value : -1);	
	tmp = intersection(head1, head2);
	printf("Intersection? %d\n", tmp ? ((t_data *)tmp->data)->value : -1);
